...
```{ojs}
//| title: "30-Day ML Ensemble Forecast Outlook"
html`
<div style="display: flex; justify-content: space-around; align-items: center; padding: 12px 15px; margin-bottom: 5px;">
...
Plot.plot({
  ...
  marks: [
    ...
    show_ci ? Plot.areaY(micro_live_c_conn, { x: "date", y1: "conf_low", y2: "conf_high", fill: "#f1c40f", fillOpacity: 0.15 }) : null,
    Plot.lineY(micro_live_c_conn, { x: "date", y: "price", stroke: "#f1c40f", strokeWidth: 3, strokeDasharray: "6 4", tip: true }),
    Plot.lineY(micro_live_g_conn, { x: "date", y: "price", stroke: "#2ecc71", strokeWidth: 3, strokeDasharray: "2 2", tip: true }),
...
# ML Deeper Dive {orientation="rows"}
...
dd_table_and_modal = {
  ...
  container.addEventListener("click", (e) => {
    ...
    if(tr && tr.dataset.cardname) {
      ...
      document.getElementById('modal-header').innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; padding-right: 30px;">
          <div style="font-size: 20px; font-weight: bold; color: #3498db;">
            ${cardname} <span style='color:#ecf0f1; font-weight:normal; font-size:16px;'>| Current: $${price.toFixed(2)}</span>
          </div>
          <div style="font-size: 16px; font-weight: normal; background: #1a252f; padding: 5px 15px; border-radius: 6px;">
            <span style='color: #bbb; text-transform: uppercase; font-size: 12px;'>Chronos (30D):</span> 
            <span style='color: #f1c40f; font-weight: bold; margin-right: 15px;'>${c_val ? "$" + c_val.toFixed(2) : "N/A"}</span>
            <span style='color: #bbb; text-transform: uppercase; font-size: 12px;'>GRU (30D):</span> 
            <span style='color: #2ecc71; font-weight: bold;'>${g_val ? "$" + g_val.toFixed(2) : "N/A"}</span>
          </div>
        </div>
      `;
      ...
      // ----------------------------------------------------
      // PLOT 2: ERROR HORIZON GRAPH ENGINE (Matches Shiny R Logic)
      // ----------------------------------------------------
      function build_error(model_name) {
         const shadow_data = full_c_fcst.filter(d => d.model === model_name && d.is_shadow);
         ...
         runs.forEach(rid => {
            ...
            // Anchor date is the last known historical price prior to the run starting
            let first_date = run_pts[0].date.getTime();
            let anchor_pt = full_c_hist.filter(h => h.date.getTime() < first_date).sort((a,b) => b.date - a.date)[0];
            let anchor_price = anchor_pt ? anchor_pt.price : null;
            
            run_pts.forEach((pt, idx) => {
               let actual_pt = full_c_hist.find(h => h.date.getTime() === pt.date.getTime());
      ...
      let err_c = build_error("Chronos");
      let err_g = build_error("GRU");
